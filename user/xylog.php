...
if(empty($imei) && !empty($mobile)){
	$auth->_main_user_db->where('mobile',$mobile);
	$query = $auth->_main_user_db->get('T_phone_info');
...
if (empty($username) || empty($imei) || empty($mobile)) {
...
//    $query = $auth->_main_user_db->query("select info from T_imeixy_log where imei='" . $imei . "'");
	  $query = $auth->_main_user_db->query("select info from `T_imeixy_log` where `imei`='" . $imei . "' and `addtime` >=  NOW() - interval 1 day order by id desc");
