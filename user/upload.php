		...
		if (isset($data['error']) && $data['error'] === 0) {
			switch ($action) {
				case 'cdma': //老人机
					...
					if ($info) {
						if (!empty($photo_path)) {
							$query = $auth->_main_user_db->query("update T_phone_info set headimag='" . $photo_path . "' where imei='" . $imei . "'");
							$auth->_main_user_db->query("update T_user_imei set headimag='" . $photo_path . "' where imei='" . $imei . "'");
						}
					} else {
						$reVal['status'] = 165;
