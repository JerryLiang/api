...
//			var_dump($query);exit;
	...
	public function verify($userid = '', $token = '')
	{
		...
		if (!empty($authVal)) {
			...
			if (is_array($val)) {
				$query = $this->_main_user_db->query("select id from T_user_info where user_name='" . $val["username"] . "' and user_pwd='" . $val["password"] . "' ");
	...
	private function _tokenCache($val = array())
	{
		...
		if (is_array($val)) {
			...
			$info = array('token' => $token, 'userid' => $val['id'], 'imei' => $val["imei"]);
			$cache_val = array(
				'userid' => $val['id'],
